...
test('usageFail', function (t) {
    var r = checkUsage(function () {
        return optimist('-left 10 -z 20'.split(' '))
            .usage('Usage: $0 -left NUM -top NUM')
            .demand(['left','top'])
            .argv;
    });
    t.same(
        r.result,
        { left : 10, z : 20, _ : [], $0 : './usage' }
    );

    t.same(
        r.errors.join('\n').split(/\n+/),
        [
            'Usage: ./usage -left NUM -top NUM',
            'Options:',
            '  -left  [required]',
            '  -top  [required]',
            'Missing required arguments: top',
        ]
    );
    t.same(r.logs, []);
    t.ok(r.exit);
...
test('usagePass', function (t) {
    var r = checkUsage(function () {
        return optimist('-left 10 -top 20'.split(' '))
            .usage('Usage: $0 -left NUM -top NUM')
            .demand(['left','top'])
            .argv;
    });
    t.same(r, {
        result : { left : 10, top : 20, _ : [], $0 : './usage' },
        errors : [],
        logs : [],
        exit : false,
    });
...
test('checkPass', function (t) {
    var r = checkUsage(function () {
        return optimist('-left 10 -top 20'.split(' '))
            .usage('Usage: $0 -left NUM -top NUM')
            .check(function (argv) {
                if (!('left' in argv)) throw 'You forgot about -left';
                if (!('top' in argv)) throw 'You forgot about -top';
            })
            .argv;
    });
    t.same(r, {
...
test('checkCondPass', function (t) {
    function checker (argv) {
        return 'left' in argv && 'top' in argv;
    }
...
test('countPass', function (t) {
    var r = checkUsage(function () {
        return optimist('1 2 3 --moo'.split(' '))
            .usage('Usage: $0 [left] [top] [z] {OPTIONS}')
            .demand(3)
            .argv;
    });
    t.same(r, {
...
test('countFail', function (t) {
    ...
    t.same(
        r.errors.join('\n').split(/\n+/),
        [
            'Usage: ./usage [left] [top] [z] {OPTIONS}',
